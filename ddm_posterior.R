...
bf_build_simulator_ddm_set <- function(
    n_trials          = 400L,
    a_range           = c(0.7, 2.5),
    v_range           = c(-3.0, 3.0),
    t0_range          = c(0.20, 0.60),
    standardize_rt    = FALSE,   # only standardize RT column; resp stays in {-1, +1}
    dt                = 0.002,   # Euler time step (s); trade off speed vs fidelity
  ...
  nT <- as.integer(n_trials)
  ex <- if (expose_split_keys) "True" else "False"
  sd_rt <- if (standardize_rt) "True" else "False"
...
def _sample_params(n):
    ...
    return np.stack([a, v, t0], axis=1)  # [n, 3]

def _simulate_case(theta_row):
    a  = float(theta_row[0])
    v  = float(theta_row[1])
    t0 = float(theta_row[2])

    # geometry: bounds at ±a/2, start at 0
    half_a = a / 2.0
    T  = _T
    dt = _dt
    tmax = _tmax
    ...
    resps = np.empty(T, dtype=np.float32)

    for t in range(T):
        x = 0.0
        # simulate until crossing or timeout
        for step in range(max_steps):
            # s=1, so increment ~ N(v*dt, sqrt(dt))
            x += v*dt + math.sqrt(dt) * rng.standard_normal()
            if x >=  half_a:
                rts[t] = (step+1) * dt + t0
                resps[t] = +1.0
                break
            ...
            # timeout: treat as max time, assign to nearest bound by sign(x)
            rts[t] = tmax + t0
            resps[t] = +1.0 if x >= 0.0 else -1.0
    ...
    X = np.stack([rts, resps], axis=1)  # [T, 2]
    return X

class DDMSetSimulator:
    def __init__(self, T): self.T = T
    def sample(self, n):
        n = _as_int(n)
        theta = _sample_params(n).astype(np.float32)   # [n,3]
        X = np.empty((n, self.T, 2), dtype=np.float32)
        for i in range(n):
            X[i] = _simulate_case(theta[i])
        return {'theta': theta, 'trials': X}
...
class KeySplitSimulator:
    ...
    def sample(self, n):
        ...
        d['v']  = th[:, 1].astype('float32')
        d['t0'] = th[:, 2].astype('float32')
        return d

simulator = DDMSetSimulator(_T)
if expose_split:
    simulator = KeySplitSimulator(simulator)

_test = simulator.sample(2)
print('[sim ok: DDM] keys:', list(_test.keys()), '| theta', _test['theta'].shape, '| trials', _test['trials'].shape)
", nT,
                                    a_range[1], a_range[2],
                                    v_range[1], v_range[2],
...
# --- Build DDM simulator ---
bf_build_simulator_ddm_set(
  n_trials       = 1200,                 # trials per case
  a_range        = c(0.5, 1.5),
  v_range        = c(-1.5, 1.5),
  t0_range       = c(0.20, 0.40),
  standardize_rt = FALSE,               # TRUE is okay too; it helps training sometimes
  dt             = 0.002,               # speed/accuracy tradeoff
  tmax           = 5.0,
  expose_split_keys = FALSE
)

# --- Build DeepSet + Flow workflow ---
bf_build_workflow_deepset(
  ...
  n_flow_layers = 6,
  flow_hidden   = c(128, 128),
  param_keys    = "theta",   # <-- theta ≡ [a, v, t0]
  data_key      = "trials"   # <-- trials ≡ [T, 2] = [rt, resp]
)
...
# --- Sample posteriors and summarise ---
post <- bf_sample_posteriors(n_test = 150, n_draws = 1200)
sumr <- bf_summarise_draws(post, param_names = c("a","v","t0"))

# --- Plot recovery & print metrics ---
out <- bf_plot_recovery(sumr, param_order = c("a","v","t0"), y_stat = "median")
out$metrics
out$patchwork
